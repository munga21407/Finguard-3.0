"""
Credit-strategy pipeline for Agent G (Credit Strategist / Report Generator).

Framework-agnostic core: cash-flow forecasting, the deterministic bankability
score, cross-agent signal folding, the LLM narrative, and PDF/Excel export
generation — none of it depends on LangGraph or ``OrchestratorState``, so it's
testable and callable without a graph. ``agents.g_reporter.make_g_reporter_node``
is a thin adapter that reads state, calls :func:`compute_credit_strategy`, and
shapes the result into a GenUI payload.
...
Pipeline:
  ...
  5. Generate a professional PDF report (reportlab) and an Excel workbook
     ...
     base64-encoded for the hub_writer to persist.
  6. Assemble AgentGOutput.
"""
...
from src.domains.intelligence.schemas import AgentGOutput
...
class CreditStrategyResult:
    ...
    q_revenue: list[float]
    q_opex: list[float]
    cross_signals: dict[str, Any] = field(default_factory=dict)
    consumed_upstream: list[str] = field(default_factory=list)
    pdf_b64: str | None = None
    xlsx_b64: str | None = None


async def compute_credit_strategy(
    *,
    raw_ledger_data: dict[str, Any] | None,
    upstream_forecast: dict[str, Any] | None,
    upstream_audit: dict[str, Any] | None,
) -> CreditStrategyResult:
    """Run the full credit-strategy pipeline (steps 1-6 of the module docstring).

    ``upstream_forecast`` / ``upstream_audit`` are Agent D's / Agent F's outputs
    when the planner ran them first (A2A consumer-read) — absent in a
    single-agent flow, in which case G stays self-contained and behaves exactly
    as it did before A2A. The deterministic bankability score is always
    ledger-derived; these signals only contextualise the narrative and surface
    as findings — they never override the numbers.
    """
    # ── 1. Load or fetch monthly cash-flow data ───────────────────────
    raw_ledger = raw_ledger_data or {}
...
{cross_instruction}"""
    ...
    return CreditStrategyResult(
        ...
        xlsx_b64=base64.b64encode(xlsx_bytes).decode("ascii") if xlsx_bytes else None,
    )
