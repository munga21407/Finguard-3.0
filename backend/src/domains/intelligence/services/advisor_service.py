"""
Agent H — Financial Advisor: framework-agnostic service layer.

RBAC role resolution, the CRM profile lookup, the RBAC-clipped evidence
prompt, the model call, widget allow-listing, and the S6-5 disclaimer/
review-gate guardrails all live here — directly unit-testable without
LangChain/LangGraph. ``agents.h_advisor`` is a thin node wrapper that calls
``build_advice`` once and shapes the message/GenUI payload. See that
module's docstring for the pipeline overview.
"""
...
async def build_advice(*, ctx: dict[str, Any], user_id: str | None) -> dict[str, Any]:
    """Runs Agent H's full pipeline (steps 1-8 of the module docstring) and
    returns everything the node needs to build its message + context update:
    ``advice`` (the ctx["advice"] payload), ``crm_profile`` (or None),
    ``valid_widgets``, and ``raw_narrative`` (the model's unmodified output,
    used only for the node's summary log line)."""
    # ── 1. RBAC resolution ────────────────────────────────────────────────────
...
"""
    ...
    return {
        "advice": advice,
        "crm_profile": crm_profile or None,
        "valid_widgets": valid_widgets,
        "raw_narrative": advisor_out.narrative_response,
    }
