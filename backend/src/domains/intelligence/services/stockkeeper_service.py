"""
Agent K — Stock Steward: framework-agnostic service layer.

Everything below is directly unit-testable without LangChain/LangGraph:
RBAC role resolution, the write-authority gate, the deterministic stock
snapshot, the CoVe audit of a proposed adjustment, and queuing an adjustment
proposal for a second authorised reviewer. ``agents.k_stockkeeper`` is a thin
node wrapper that calls ``run_stock_analysis`` once and shapes the summary
message — see that module's docstring for the pipeline overview.
"""
...
from src.domains.intelligence.schemas import AgentKOutput, ProposedStockAction
...
async def run_stock_analysis(
    *, ctx: dict[str, Any], user_id: str | None, query: str
) -> dict[str, Any]:
    """Orchestrates Agent K's full pipeline (steps 1-6 of the module docstring)
    and returns the ``inventory_analysis`` payload the node writes to context."""
        ...
        if isinstance(action, dict) and action.get("product_ref"):
            ...
            if movement_type.lower() == MovementType.ADJUSTMENT.value:
                cove_evidence = {
                    ...
                    "low_stock": [
                        i.model_dump(mode="json") for i in low_stock[:_MAX_REORDER_ITEMS]
                    ],
    ...
    if len(low_stock) == 0:
        narrative = "✅ No products are at or below their reorder level. " + narrative
