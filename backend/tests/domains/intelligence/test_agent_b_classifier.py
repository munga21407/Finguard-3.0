...
def _fake_client_returning(result: BatchClassificationResult) -> Any:
...
async def test_empty_entries_short_circuits() -> None:
    # No entries → no Gemini call, empty result.
    assert await _classify_via_gemini([]) == []


@pytest.mark.asyncio
async def test_missing_entry_id_backfilled_as_other(monkeypatch: pytest.MonkeyPatch) -> None:
    # Gemini returns only e1; e2 must be backfilled as "other"/0.0.
    returned = BatchClassificationResult(
        classifications=[TransactionClassification(entry_id="e1", category=_VALID_CATEGORY, confidence=0.9)]
    )
    monkeypatch.setattr(b_classifier, "get_gemini_client", lambda: _fake_client_returning(returned))
...
async def test_out_of_taxonomy_category_coerced(monkeypatch: pytest.MonkeyPatch) -> None:
    returned = BatchClassificationResult(
        classifications=[TransactionClassification(entry_id="e1", category="not_a_real_category", confidence=0.8)]
    )
    monkeypatch.setattr(b_classifier, "get_gemini_client", lambda: _fake_client_returning(returned))
